...
- **Layout.** `crates/momr-core` is the engine (no UI, compiles for every target), `crates/momr-platform` holds one module per OS seam, `src/` is the GTK shell (`src/ui.rs` is the window and state machine), `apps/momr-appkit` is the AppKit shell, which runs the `momr` binary (`finish`, `transcribe`) rather than linking the core. Platform work lands in `momr-platform`.
- Every Rust source file opens with a module doc that says what the module does and why it is built that way. Read it before editing; it is the source of truth for the module.
...
- **Linux code goes when its macOS replacement lands.** `cfg(target_os)` lives only in `momr-platform` (D25), and only for a target someone runs; an unscheduled target gets a `compile_error!`, not a guessed branch. The plan for each seam says what replaces what.
...
- **Dependencies stay few.** `ureq`, `std::process::Command`, hand-parsed `serde_json`, no async runtime. Add a crate only when std or GLib cannot do the job, and say why in `Cargo.toml` the way the GPU feature is annotated. Apple APIs go in Swift (`helpers/momr-audio`, `apps/momr-appkit`), not in Rust bindings.
