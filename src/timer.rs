...
pub fn clock_time(at: i64) -> String {
    let Ok(when) = glib::DateTime::from_unix_local(at) else {
        return String::new();
    };
    let today = glib::DateTime::now_local()
        .ok()
        .is_some_and(|now| now.day_of_year() == when.day_of_year() && now.year() == when.year());
...
pub fn next_occurrence(hour: i32, minute: i32, now: &glib::DateTime) -> Option<i64> {
    let today = glib::DateTime::from_local(
        now.year(),
        now.month(),
        now.day_of_month(),
        hour,
    ...
    .ok()?;
    let candidate = if today.to_unix() > now.to_unix() {
        today
    } else {
        today.add_days(1).ok()?
    };
    Some(candidate.to_unix())
}
...
mod tests {
    ...
    fn next_occurrence_is_today_or_tomorrow() {
        let now = glib::DateTime::from_local(2026, 9, 25, 16, 0, 0.0).unwrap();
        let later = next_occurrence(17, 30, &now).unwrap();
        assert_eq!(later - now.to_unix(), 90 * 60);
        let earlier = next_occurrence(15, 0, &now).unwrap();
        assert_eq!(earlier - now.to_unix(), 23 * 3600);
        // The same minute counts as passed: it would start at once.
        assert_eq!(
            next_occurrence(16, 0, &now).unwrap() - now.to_unix(),
            24 * 3600
        );
    }
}
